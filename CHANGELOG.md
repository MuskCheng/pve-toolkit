# 更新日志

所有重要的更改都将记录在此文件中。

## [V0.9.1] - 2026-03-23

### 修复

- **OpenClaw 原生安装前端构建问题**
  - 新增 Bun 安装步骤（openclaw 构建依赖）
  - 增加前端资源构建验证（检查 dist/control-ui/index.html）
  - 构建失败时自动重试 `pnpm ui:install && pnpm ui:build`
  - 优化构建日志输出，显示关键信息
  - systemd 服务添加 Bun PATH 环境变量

## [V0.9.0] - 2026-03-23

### 新增

- **LXC 容器管理 - 应用安装菜单**
  - 新增「应用安装」子菜单，集中管理 LXC 容器应用安装
  - 支持 OpenClaw、Lucky、DPanel 等应用一键安装

- **OpenClaw 安装功能重构**
  - 新增原生安装方式（推荐），直接在 LXC 容器内运行，解决前端资源缺失问题
  - 保留 Docker 安装方式作为备选
  - 原生安装自动构建前端资源（pnpm ui:build），确保 Control UI 正常工作
  - 自动创建 systemd 服务，支持开机自启
  - 自动生成 Gateway Token

### 优化

- **菜单结构调整**
  - 将应用安装（OpenClaw/Lucky/DPanel）从 Docker 管理移至 LXC 容器管理
  - Docker 管理菜单精简为：安装 Docker、换源、容器管理
  - 更清晰的功能分类：Docker 管理专注 Docker 本身，应用安装在 LXC 层面统一管理

### 修复

- **OpenClaw 前端启动问题**
  - 修复使用 Docker 镜像安装后前端无法启动的问题
  - 原生安装方式从源码构建，确保前端资源正确生成

## [V0.7.1] - 2026-03-13

### 新增

- **Docker 管理**
  - 新增 Lucky 大吉安装功能
  - 支持 Host/Bridge 网络模式选择
  - 自定义配置目录
  - 完整的安全提示（外网访问时限、安全入口设置）

### 优化

- **Docker 管理菜单**
  - 调整菜单顺序，新增 Lucky 安装选项

## [V0.7.0] - 2026-03-13

### 新增

- **AGENTS.md**
  - 新增项目说明文件，用于 AI 助手上下文

## [V0.6.0] - 2026-02-24

### 新增

- **换源工具**
  - 新增南京大学源、腾讯云源选项
  - 新增 Ceph 源配置功能
  - 动态检测 Debian/PVE 版本
  - 根据选择的镜像站进行网络检测
  - 增强企业源处理（支持 .list 文件）

- **Git Hook**
  - 新增 post-merge hook，每次 git pull 后自动清理旧文件

### 优化

- **Docker 安装**
  - 使用阿里云镜像源加速 Docker CE 安装
  - 移除 apt update 步骤，直接安装加速
  - 添加 docker.io 作为后备安装方案
  - pip 安装使用清华 pip 镜像源

### 修复

- **LXC 容器列表**
  - 修复颜色变量转义（单引号改为 ANSI-C quoting）
  - 修复格式说明符转义（echo 改为 printf）
  - 每个字段单独着色解决对齐问题
  - 移除 Docker 端口列
  - 修复 IPv4 地址获取（排除回环接口）

## [V0.5.35] - 2026-02-24

### 修复

- **LXC 容器列表**
  - 修复颜色显示异常（echo -e 代替 printf）

## [V0.5.34] - 2026-02-24

### 修复

- **LXC 容器列表**
  - 修复 ANSI 颜色显示异常（使用 printf %b 格式说明符）

## [V0.5.33] - 2026-02-24

### 修复

- **LXC 容器列表**
  - 修复 ANSI 颜色转义序列显示异常（printf 改为 echo -e）

## [V0.5.32] - 2026-02-24

### 修复

- **LXC 容器列表**
  - 修复列顺序显示错误（名称与状态位置互换）

## [V0.5.31] - 2026-02-23

### 移除

- **LXC 管理**
  - 移除「安装常用工具」菜单项
- **Docker 管理**
  - 移除「安装 Docker (离线包)」菜单项
- **GitHub Actions**
  - 移除 sync-offline.yml 工作流（不再构建离线包）

## [V0.5.30] - 2026-02-23

### 修复

- **Docker 镜像源更新**
  - 更新为 2026 年 2 月最新可用镜像源
  - 毫秒镜像 (docker.1ms.run) - 推荐
  - 轩辕镜像免费版
  - 耗子面板
  - 中科大镜像

## [V0.5.29] - 2026-02-23

### 优化

- **LXC 容器列表显示增强**
  - 新增 IP 地址显示
  - 新增 Docker 端口显示
  - 新增特权容器标识
  - 优化显示格式和对齐
  - 颜色区分运行/停止状态

## [V0.5.28] - 2026-02-23

### 修复

- **屏蔽订阅提示功能**
  - 修复正则表达式匹配问题（转义特殊字符）
  - 新增三重策略：res.data.status / !== 'active' / Perl多行匹配
  - 提高不同 PVE 版本的兼容性

## [V0.5.27] - 2026-02-23

### 优化

- **架构调整：前置检测移至 install.sh**
  - install.sh: 完整前置检测（Root权限/PVE环境/依赖包/PVE版本）
  - pve-tool.sh: 简化，移除重复检测
  - 支持参数透传（--debug）

## [V0.5.26] - 2026-02-23

### 新增

- **系统检测增强**
  - 新增依赖包检查（自动安装 curl）
  - 新增 PVE 版本风险提示（非 PVE9 警告）
  - 新增调试模式（--debug 参数）
  - 新增破坏性操作拦截函数

- **换源前置检查**
  - 拦截非 PVE9 环境执行换源
  - 网络连接检测

## [V0.5.25] - 2026-02-23

### 优化

- **换源功能重构**
  - 采用 DEB822 格式（PVE 9.x 推荐）
  - 新增 CT 模板源替换（加速 LXC 模板下载）
  - 新增安全源选择（官方/镜像站）
  - 统一备份机制

- **内核管理扩展**
  - 新增查看可用内核列表
  - 新增安装指定内核
  - 新增设置默认启动内核

- **备份机制增强**
  - 统一备份到 /var/backups/pve-tools/
  - 备份文件带时间戳

## [V0.5.24] - 2026-02-23

### 修复

- **屏蔽订阅提示功能**
  - 添加备用策略（策略B），支持更多 PVE 版本

### 优化

- 菜单显示已调整（系统管理第一顺位）
- 屏蔽订阅功能已存在

## [V0.5.23] - 2026-02-23

### 修复

- **屏蔽订阅提示功能**
  - 修复无效的 sed 命令
  - 使用更可靠的替换方法

## [V0.5.22] - 2026-02-23

### 新增

- **屏蔽订阅提示**
  - 新增功能：屏蔽 PVE 网页登录时的"无效订阅"弹窗
  - 支持一键屏蔽/恢复

### 调整

- **主菜单顺序调整**
  - 系统管理调整至第一顺位

## [V0.5.21] - 2026-02-23

### 修复

- **Docker 容器端口显示**
  - 修复端口无法正确显示的问题
  - 优化显示格式，仅显示 Host 端口

## [V0.5.20] - 2026-02-23

### 优化

- **简化容器创建流程**
  - 移除创建容器后询问是否预装 Docker 的提示
  - 用户可通过 Docker 管理或安装常用工具功能按需安装

- **安装常用工具提示**
  - 添加提示信息："请先创建 LXC 容器，或使用已有容器"

## [V0.5.19] - 2026-02-23

### 新增

- **安装常用工具菜单**
  - LXC 容器管理菜单新增"安装常用工具"选项
  - 新增 Lucky V2 一键安装功能
  - 预留更多工具扩展位置

- **Lucky V2 安装功能**
  - 支持自定义端口、配置目录、容器名称
  - 默认值回车确认，简化操作
  - 自动检测 Docker 环境，未安装时提示安装
  - 优先从离线包加载镜像，失败时回退在线拉取
  - 安装完成后显示访问地址

## [V0.5.18] - 2026-02-23

### 新增

- **Docker 离线安装功能**
  - Docker 管理菜单新增"安装 Docker (离线包)"选项
  - 支持从 GitHub Releases 下载离线包（含 Docker、Docker Compose、Lucky V2）
  - 支持 GitHub 代理加速下载（国内推荐）
  - 支持本地离线包安装

- **自动离线包构建工作流**
  - 添加 GitHub Actions 工作流自动检测 Docker/Lucky 更新
  - 有更新时自动构建并发布到 Releases
  - Release 说明中包含各组件版本信息

### 变更

- **系统要求调整**
  - 最低 PVE 版本从 9.0 调整为 9.1
  - 最低 Debian 版本保持为 13 (Trixie)

## [V0.5.17] - 2026-02-23

### 新增

- **创建容器时选择容器类型**
  - 支持选择特权容器或无特权容器
  - 显示两种容器类型的功能差异说明
  - 默认选择特权容器（Docker 兼容性最佳）

- **容器类型转换功能**
  - 容器操作菜单新增"转换容器类型"选项
  - 支持特权容器 ↔ 无特权容器互转
  - 特权→无特权转换时显示警告并自动转换文件所有权
  - 转换过程自动停止/启动容器

- **GitHub Actions 自动发布**
  - 添加 CI 工作流，VERSION 文件更新时自动创建 Release

## [V0.5.16] - 2026-02-22

### 新增

- **网络配置详细显示**
  - 新增 CIDR 转子网掩码函数 `cidr_to_netmask()`
  - 支持显示子网掩码、网络地址、广播地址
  - DHCP 模式下自动获取容器实际 IP 地址
  - 容器未运行时显示提示信息

### 优化

- **网络配置显示格式**
  - 使用分割线替代表格框线，避免终端显示错位
  - 简化输出格式，提高兼容性

## [V0.5.15] - 2026-02-22

### 新增

- **Docker 容器管理功能**
  - Docker 管理菜单新增"容器管理"选项
  - 查看容器列表、状态、详情
  - 启动/停止/重启容器
  - 查看容器日志（最近100行）
  - 进入容器终端（自动检测 sh/bash）
  - 删除容器
  - 清理无用容器和镜像（docker system prune）

- **LXC 容器网络配置修改**
  - 容器操作菜单新增"修改网络配置"选项
  - 支持设置静态 IP 地址
  - 支持切换到 DHCP 模式
  - 显示当前网络配置（接口、网桥、IP、网关）
  - 修改后可选自动重启容器使配置生效

### 修复

- **删除容器逻辑错误**
  - 修复删除确认逻辑，确保用户确认后才执行删除操作
  
- **Docker 换源配置问题**
  - 修复变量替换问题，确保镜像源地址正确写入配置文件

- **部署结果验证**
  - 添加 Docker Compose 部署结果验证
  - 部署失败时显示错误信息

- **代码规范**
  - 定义 WHITE 颜色变量

## [V0.5.14] - 2026-02-22

### 新增

- **Docker 换源功能**
  - Docker 管理菜单新增"换源"选项
  - 支持多个国内镜像源：阿里云、中科大、网易、腾讯云、Docker中国
  - 支持自定义镜像源
  - 自动配置 /etc/docker/daemon.json
  - 自动重启 Docker 服务并验证

### 修复

- **Docker Compose 路径问题**
  - 使用完整路径调用 docker-compose（/usr/local/bin/docker-compose）
  - 解决容器内 PATH 未包含 /usr/local/bin 导致命令找不到的问题

## [V0.5.13] - 2026-02-22

### 修复

- **Docker Compose 安装稳定性修复**
  - 使用固定版本 v2.24.0 替代动态获取最新版本（避免 v5.x 文件名不兼容）
  - 增加下载文件完整性验证：
    - 检查文件大小 > 1MB
    - 检查文件头是否为 HTML（避免 404 页面）
    - 使用 `file` 命令验证 ELF 可执行文件格式
    - 验证 `--version` 输出包含 "Docker Compose"
  - 验证失败时自动删除无效文件

## [V0.5.12] - 2026-02-22

### 修复

- **容器内命令检测修复**
  - 使用 `bash -lc` 替代 `bash -c`，确保加载用户环境变量
  - 添加文件路径检测作为备用（`/usr/bin/docker`, `/usr/local/bin/docker-compose`）
  - 解决 LXC 容器中 PATH 未正确设置导致命令检测失败的问题

- **Docker Compose 安装改进**
  - 添加 curl/wget 安装状态变量检测
  - 显示 curl/wget 安装结果
  - 使用绝对路径验证 docker-compose 安装

## [V0.5.11] - 2026-02-22

### 修复

- **Docker 检测逻辑优化**
  - 使用 `command -v docker` 检测，更可靠
  - 添加服务启动尝试逻辑
  - 解决已安装 Docker 但检测不到的问题

- **Docker Compose 安装重构**
  - 版本获取移至容器内执行（而非宿主机）
  - 添加 curl/wget 安装验证
  - 移除 apt 安装方式（Debian Trixie 无 docker-compose-plugin 包）
  - 改进下载逻辑：添加超时参数，使用 -f 强制失败检测
  - 支持 docker-compose V1 和 docker compose V2 plugin

## [V0.5.10] - 2026-02-22

### 重构

- **主菜单精简**
  - 删除备份管理、VM 管理、常用工具等非核心功能
  - 主菜单从 8 项精简为 3 项
  - 代码行数从 1652 行减少至 ~1000 行

### 新增

- **容器操作子菜单**
  - LXC 容器管理新增"容器操作"子菜单
  - 整合控制台/启动/停止/重启/克隆/修改资源功能

- **系统清理增强**
  - 新增多种清理选项：apt 缓存、旧内核、临时文件
  - 支持单独或完整清理

### 优化

- **Docker 环境检测**
  - 支持 Docker Compose V1/V2 兼容检测
  - 部署前自动检查并安装 Docker 环境

- **LXC 创建优化**
  - 创建容器时询问是否预装 Docker 环境

## [V0.5.7] - 2026-02-22

### 修复

- **Docker 安装**
  - 修复 Docker Compose 安装失败问题
  - 改进安装逻辑，支持多种安装方式（apt → pip → 二进制）

## [V0.5.6] - 2026-02-22

### 新增

- **LXC 创建容器**
  - 增加磁盘空间建议提示
  - 默认磁盘大小从 20GB 调整为 8GB

## [V0.5.5] - 2026-02-22

### 新增

- **Docker 管理功能增强**
  - 新增 Docker 部署向导（使用 docker run 部署单个容器）
  - 支持在线选择热门镜像（Nginx, MySQL, PostgreSQL, Redis, MongoDB, etc.）
  - Docker Compose 部署向导新增自定义部署（粘贴 docker-compose.yml）
  - 所有部署向导自动检查并安装 Docker/Docker Compose

### 优化

- Docker 管理菜单结构调整
- 安装 Docker 时自动包含 Docker Compose

## [V0.5.4] - 2026-02-22

### 修复

- **LXC 创建容器**
  - 修复模板名称包含状态信息问题

## [V0.5.3] - 2026-02-22

### 修复

- **LXC 创建容器**
  - 修复模板名称错误导致容器创建失败

## [V0.5.2] - 2026-02-22

### 新增

- **LXC 创建容器优化**
  - 创建容器默认使用最新模板，移除手动选择

## [V0.5.1] - 2026-02-22

### 新增

- **LXC 创建容器优化**
  - 下载新模板后自动清理旧版 Debian 模板
  - 创建容器前显示当前 LXC 容器列表

## [V0.5.0] - 2026-02-22

### 新增

- **LXC 创建容器优化**
  - 自动检测并下载 Proxmox 官方镜像库中最新 Debian 模板
  - 创建容器时自动使用最新模板
  - 本地模板已存在时跳过下载
  - 用户仍可手动选择其他模板

## [V0.38] - 2026-02-21

### 新增

- **LXC 创建容器优化**
  - 创建容器前显示已下载的 LXC 模板列表
  - 支持用户选择使用哪个模板
  - 默认使用 debian-12-standard 模板
- **Docker 管理增强**
  - 新增独立的 Docker 管理子菜单
  - 安装 Docker / Docker Compose
  - Docker Compose 部署向导（预置9种常用服务模板）
  - 一键升级镜像功能
- **版本管理优化**
  - 创建 VERSION 文件统一管理版本号
  - 启动时自动查询 GitHub 最新版本

## [V0.28] - 2026-02-21

### 重构

- 移除交互模式，改为纯命令行模式
- 解决管道执行的闪屏问题
- 代码精简至约 150 行

## [V0.18] - 2026-02-21

### 修复

- 安装脚本对 apt 镜像源错误更宽容
- 修复 Docker 安装兼容性问题（支持多源回退：官方源→阿里云源→系统自带）

## [V0.1] - 初始版本

### 新增

- **主框架**
  - 交互式菜单界面
  - 命令行模式支持
  - ASCII Logo 显示
- **备份管理模块**
  - 列出所有 VM/LXC 备份
  - 创建备份（支持 snapshot 模式）
  - 清理过期备份
  - 恢复备份
- **系统监控模块**
  - 系统状态概览（CPU/内存/磁盘/运行时间）
  - 虚拟机状态列表
  - LXC 容器状态列表
  - 资源阈值检查告警
  - 网络接口状态
  - 系统日志查看
- **LXC 容器管理模块**
  - 列出所有容器
  - 创建容器（支持自定义配置）
  - 启动/停止/重启容器
  - 删除容器（支持强制删除）
  - 容器详情查看
  - Docker 安装（使用阿里云镜像源加速）
  - Docker Compose 安装（使用 pip）
  - Docker 镜像源切换（6 个国内镜像源）
  - Docker 容器管理（镜像搜索、拉取、运行）
- **系统管理模块**
  - 显示镜像源配置
  - 切换镜像源（6 个国内镜像源）
  - 禁用 PVE 企业源
  - 配置 PVE 社区源
  - 系统更新
  - 系统清理
  - 系统信息显示
